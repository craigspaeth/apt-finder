...
const scrapeUrbanEdge = async () => {
  if (!'urbanedge'.match(TARGET_SCRAPERS)) return
...
const scrapeListUI = async ({
  ...
  try {
    ...
    const listings = _.compact(
      $(liSelector)
        .map((i, el) => {
          const href = _.isString(thumbnail($(el)))
            ? thumbnail($(el))
            : thumbnail($(el)) && thumbnail($(el)).length
                ? thumbnail($(el)).get(0).tagName === 'img'
                    ...
                    : thumbnail($(el)).css('background-image')
                        ...
                        : null
                : null
          const thumbnailSrc = fullUrl(url, href)
          const $link = link($(el)) && link($(el)).first()
          const $rent = rent($(el)) && rent($(el)).first()
          return {
            href: $link ? fullUrl(url, $link.attr && $link.attr('href')) : url,
