...
const scrape = async ({ url, liSelector, link, thumbnail, rent, noFee }) => {
  ...
  try {
    ...
    const listings = _.compact(
      $(liSelector)
        .map((i, el) => {
          // Grab the thumbnail src from either an img src or background image
          ...
          // Return a normalize listing data model
          return {
            href: fullUrl(link($(el)).attr && link($(el)).attr('href')),
            thumbnail: thumbnailSrc,
            rent: accounting.unformat(rent($(el)).text()),
            noFee: !!noFee($(el))
          }
        })
        .get()
